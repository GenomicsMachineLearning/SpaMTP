...
#'
#' @examples
#' spa_network(SpaMTP, ident = "Custom_ident", regpathway = regpathway,
#'             DE.list = DE.list, selected_pathways = selected_pathways)
...
spa_network  = function(SpaMTP,
  ...
  check_topology_existence = function(type,
    ...
    temp_db = c()
    if (type == "wiki") {
      wikiids = unlist(lapply(humanwiki_ramp, function(x) {
        return(x[["id"]])
      }))
      ...
      reactomeids = unlist(lapply(humanReactome_ramp, function(x) {
        return(x[["id"]])
      }))
      index = which((tolower(names(
        humanReactome_ramp
      ...
      keggids = unlist(lapply(humankegg_ramp, function(x) {
        return(x[["id"]])
      }))
      index = which((tolower(names(
        humankegg_ramp
      ...
      hmdbids = unlist(lapply(humansmp_ramp, function(x) {
        return(x[["id"]])
      }))
      index = which((tolower(names(
        humansmp_ramp
...
document.getElementById("saveButton").addEventListener("click", function () {
  ...
  returnname = paste0(ident, "_", Sys.Date())

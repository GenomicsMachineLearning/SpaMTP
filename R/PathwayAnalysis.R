...
#' @param ident A factor vector where each levels indicates the different clustered regions in tissue
...
#' @param exclude_background A boolean value indicates whether the background should be remove during deg analysis.
#' @param ... The other parameters goes to Seurat methods
...
#'
#' @examples
#' # SpaMTP = PathwaysPerRegion(SpaMTP, polarity = "positive")
...
PathwaysPerRegion = function(SpaMTP,
  ...
  if (!is.null(ident)) {
    ...
    if ((("metabolites" %in% analyte_types)&length(ident)!= nrow(mass_matrix)) |
        (("gene" %in% analyte_types)&length(ident)!= nrow(gene_matrix)) ) {
      stop(
        "Please make sure the input cluster is a vector has same length as the number of spots which contains spatial information and is a factor"
      )
    } else{
      
      assignment = ident
      cluster = levels(ident)
      sscs = "Custom_clustering"
      user_input = 1
      sscs[user_input]
  ...
  if (is.null(background_cluster)) {
    #assignment = as.factor(gsub("\\,.*", "", SpaMTP@meta.data[which(names(SpaMTP@meta.data) == sscs[user_input])][, 1]))
    assignment[which(is.na(assignment))] = sample(levels(assignment), size=1)
    cluster = levels(assignment)
    coordnate = sapply(
      SpaMTP@meta.data[, which(grepl(
        colnames(SpaMTP@meta.data),
  ...
  if(exclude_background == F){
    ...
    temp_spa = suppressWarnings({SpaMTP[,which(background_cluster==0)]})
    temp_spa@meta.data[[u]] = indices[which(background_cluster==0)]
    Idents(temp_spa) = u
    DE_met <- FindAllMarkers(temp_spa ,
                             assay = "SPM",
                             only.pos = F,
                             test.use = test_use,
                             ...)
    ...
    db_3 = merge(db_3 , DE_met, by = "mz_name")
    verbose_message(message_text = "Calculating transcriptomics differentially expressed cluster ..." , verbose = verbose)
    DE_rna <- FindAllMarkers(temp_spa ,
                             assay = "SPT",
                             only.pos = F,
                             test.use = test_use,
                             ...
  ...
  if ("metabolites" %in% analyte_types){
    SpaMTP@misc[[paste0("DE_",
                        ...
                        sscs[user_input],
                        "_",
                        paste0(c("Baseline",background_clu), collapse = "."),
                        "_",
                        paste0(c("ROI",  unique(cluster[-which(cluster %in% background_clu)])), collapse = "."),
                        "_",
                        ifelse(exclude_background,"baseexcluded","baseincluded"))]] = db_3 
  }
  if("gene" %in% analyte_types){
    SpaMTP@misc[[paste0("DE_",
                        spatial_transcriptomic_assay,
                        ...
                        paste0(c("ROI", unique(cluster[-which(cluster %in% background_clu)])), collapse = "."),
                        "_",
                        ifelse(exclude_background,"baseexcluded","baseincluded"))]] = source_gene 
  }
  ...
  for (i in 1:(length(non_bac_cluster) + 1)) {
    ...
    if (i == (length(non_bac_cluster) + 1)) {
      if (all(background_cluster == 0) |exclude_background == T) {
    ...
    addtional_entry = do.call(rbind, lapply(1:nrow(gsea_result), function(x) {
      ...
      temp_ref = db_3[which(db_3$ramp_id %in% temp), ] %>% dplyr::mutate(adduct_info = paste0(observed_mz, "[", Adduct, "]")) %>% dplyr::filter(!duplicated(adduct_info)) %>% dplyr::filter(!duplicated(ramp_id))
      ...
      return(
        data.frame(
          ...
          met_regulation = paste0(ifelse(ranks[which((names(ranks) %in% temp) & (grepl(names(ranks), pattern = "RAMP_C")))] >= 0, "↑", "↓"), collapse = ";"),
          rna_regulation = paste0(ifelse(ranks[which((names(ranks) %in% temp)& (grepl(names(ranks), pattern = "RAMP_G")))] >= 0, "↑", "↓"), collapse = ";")
        )
      )
    }))
  ...
  close(pb3)
  gsea_all_cluster_sig = gsea_all_cluster %>% dplyr::group_by(pathway) %>% dplyr::mutate(
    Significance = ifelse(
      pval <= 0.05,
  ...
  gsea_all_cluster_return =   gsea_all_cluster_sig %>% dplyr::filter(!duplicated(paste0(pathwayName,Cluster_id)))
  gsea_all_cluster_return = merge(gsea_all_cluster_return, pathway, by = "pathwayName")
  ########################################################
  #Plot#
  SpaMTP@misc[[paste0("set_enriched",
                      "_",
                      sscs[user_input],
                      "_",
                      paste0(c("Baseline",background_clu), collapse = "."),
                      ...
                      paste0(c("ROI",unique(cluster[-which(cluster %in% background_clu)])), collapse = "."),
                      "_",
                      ifelse(exclude_background,"baseexcluded","baseincluded"))]] = gsea_all_cluster_return
  gc()
  return(SpaMTP)
}

...
Function used by MapSpatialOmics to align SM data to ST spots with lower
resolution (i.e. Visium Spots)
...
- ST.image:
  ...
  stored in the Visium object (default = "slice1").
...
- ST.scale.factor:
  ...
  used and no scaling will be performed (default = "hires").
...
- overlap.threshold:
  ...
  visium spot will be assigned a match (default = 0.2).
...
- merge.unique.metadata:
  ...
  only store unique values in the metadata. Note: `add.metadata` must be
  set to `TRUE` for this functionality to be implmented (default =
  TRUE).
...
equivalent Spatial Transcripomics (Visium) spots.

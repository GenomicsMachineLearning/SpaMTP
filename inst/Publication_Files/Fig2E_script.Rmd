...
df <- new_df %>%   # replace with your object name
  mutate(
    cluster = if_else(count < 0, 6L, cluster),
    count   = abs(count)
  )
df <- subset(df, count != 0)
df <- subset(df, Lipid.Maps.Category != "Non-Lipids")
...
p <- ggplot(category_counts,aes(fill=cluster, y=count, x=id))+
   ...
   xlim(0,dim(category_counts)[1])+ylim(-10,max(category_counts$count))+geom_text(data=label_data, aes(x=id, y = count+1,label=Lipid.Maps.Category),size = 4, angle = label_data$angle)+

...
#annot <- subset(integrated.data, subset = annotations %in% c("CP_intact", "ACB_intact"))
#sm <- subset(integrated.data, subset = SM_clusters=="3")
#st <- subset(integrated.data, subset = ST_clusters %in% c("0", "4"))
#int <- subset(integrated.data, subset = integrated_clusters %in% c("4","3"))

### Updated annotations
annot <- subset(integrated.data, subset = annotations %in% c("CP_intact", "ACB_intact"))
sm <- subset(integrated.data, subset = gpca_SPM %in% c("1","5"))
st <- subset(integrated.data, subset = gpca_SPT %in% c("7", "5", "9"))
int <- subset(integrated.data, subset = integrated_clusters_2 %in% c("2","10"))

integrated.data$annot <- ifelse(integrated.data$annotations %in% c("CP_intact", "ACB_intact"), "T","F")
integrated.data$sm <- ifelse(integrated.data$gpca_SPM %in% c("1","5"), "T","F")
integrated.data$st <- ifelse(integrated.data$gpca_SPT %in% c("7", "5", "9"), "T","F")
integrated.data$int <- ifelse(integrated.data$integrated_clusters_2 %in% c("2"), "T","F")
...
correlation_df <- lapply(list_names, function(x){
  suppressWarnings({
  cor <- FindCorrelatedFeatures(integrated.data, ident = x, ST.assay = "SPT", SM.assay = "SPM", ST.slot = "counts", SM.slot = "counts", nfeatures = NULL)
  })
  cor[cor$ident == "T",]
})
...
scaled_data <-scaled_data[rev(colnames(scaled_data))]
...
new_df <- df[c("Dopamine.Correlation", " % Dopamine Spots")]
new_df[c(" % Dopamine Spots")] <- new_df[c(" % Dopamine Spots")]/100
new_df$ARI <- c( 1, 0.2292700, 0.3450286,0.3871174)
new_df$Group <- rownames(new_df)
long_df <- new_df[c(2:4),] %>%
...
ggplot(long_df, aes(x = Metric, y = Value, fill = Group)) +
  # Alternating background rectangles
   
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  theme_minimal() +
  labs(title = "Comparison of Metrics by Group",
       x = "Metric", y = "Value") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

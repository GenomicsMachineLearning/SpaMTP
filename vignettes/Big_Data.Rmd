...
<div style="margin-left: auto;
            margin-right: auto">
```{r, echo=FALSE}
results_url <- "https://zenodo.org/records/17288700/files/results_spotted.csv?download=1"
results_path <- "vignette_data_files/Spotted_Liver/results_spotted.csv"
if (!file.exists(results_path)) {
  dir.create(dirname(results_path), recursive = TRUE, showWarnings = FALSE)
  tryCatch(
    download.file(results_url, results_path, mode = "wb", quiet = TRUE, method = "libcurl"),
    error = function(e) message("Download failed: ", conditionMessage(e))
  )
}
df <- read.csv(results_path, row.names = 1)
df %>%
...
```{r, include=FALSE}
options(timeout = 600)
spotted_url <- "https://zenodo.org/records/17288700/files/spotted_small_ROI.RDS?download=1"
spotted_path <- "vignette_data_files/Spotted_Liver/spotted_small_ROI.RDS"
if (!file.exists(spotted_path)) {
  dir.create(dirname(spotted_path), recursive = TRUE, showWarnings = FALSE)
  tryCatch(
    download.file(spotted_url, spotted_path, mode = "wb", quiet = TRUE, method = "libcurl"),
    error = function(e) message("Download failed: ", conditionMessage(e))
  )
}
spotted_small <- if (file.exists(spotted_path)) readRDS(spotted_path) else NULL
```


```{r, warning=FALSE, message=FALSE}
if (!is.null(spotted_small)) {
  ...
  message("Skipping plot: spotted_small_ROI.RDS not available.")
}
```
...
```{r, warning=FALSE, message=FALSE}
if (inherits(spotted_small, "Seurat")) {
  spotted_small <- SeuratObject::UpdateSeuratObject(spotted_small)
}

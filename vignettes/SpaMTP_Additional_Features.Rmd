...
bladder_url <- "https://zenodo.org/records/17246684/files/bladder_annotated.RDS?download=1"
bladder_path <- "vignette_data_files/Mouse_Urinary_Bladder/bladder_annotated.RDS"
if (!file.exists(bladder_path)) {
  dir.create(dirname(bladder_path), recursive = TRUE, showWarnings = FALSE)
  tryCatch(
    download.file(bladder_url, bladder_path, mode = "wb", quiet = TRUE, method = "libcurl"),
    error = function(e) message("Download failed: ", conditionMessage(e))
  )
}
bladder_annotated <- readRDS(bladder_path)
...
```{r, fig.width=11, fig.height=5, warning=FALSE, message=FALSE, dpi=72}
...
```{r, fig.width=7, fig.height=3, warning=FALSE, message=FALSE,out.width="80%", dpi=72}
...
```{r, fig.width=22, fig.height=5, warning=FALSE, message=FALSE, dpi=96}
...
```{r, fig.width=7, fig.height=3, out.width="80%",warning=FALSE, message=FALSE, dpi=96}
...
```{r, fig.width=20, fig.height=5, warning=FALSE, message=FALSE, dpi=96}
...
```{r, fig.width=20, fig.height=10, warning=FALSE, message=FALSE, dpi=96}
...
```{r, fig.width=7, fig.height=3, out.width="80%",warning=FALSE, message=FALSE, dpi=72}
...
```{r, include=FALSE}
bladder_he_url <- "https://zenodo.org/records/17246684/files/bladder_HE.RDS?download=1"
bladder_he_path <- "vignette_data_files/Mouse_Urinary_Bladder/bladder_HE.RDS"
if (!file.exists(bladder_he_path)) {
  dir.create(dirname(bladder_he_path), recursive = TRUE, showWarnings = FALSE)
  tryCatch(
    download.file(bladder_he_url, bladder_he_path, mode = "wb", quiet = TRUE, method = "libcurl"),
    error = function(e) message("Download failed: ", conditionMessage(e))
  )
}
bladder_HE <- readRDS(bladder_he_path)

bladder_HE <- UpdateSeuratObject(bladder_HE) # Only Required if SeuratObjects => 5.0.2
```  
...
```{r fig.width=7, fig.height=7, warning=FALSE, message=FALSE, out.width="80%", dpi=72}
...
```{r, eval=FALSE, fig.width=10, fig.height=10, warning=FALSE, message=FALSE, dpi=96}
...
<iframe src="vignette_data_files/Mouse_Urinary_Bladder/3d_plot1.html" width="100%" height="400px"></iframe>
...
<iframe src="vignette_data_files/Mouse_Urinary_Bladder/3d_plot2.html" width="100%" height="400px"></iframe>
...
<iframe src="vignette_data_files/Mouse_Urinary_Bladder/3d_plot3.html" width="100%" height="400px"></iframe>
...
<iframe src="vignette_data_files/Mouse_Urinary_Bladder/3d_plot4.html" width="100%" height="400px"></iframe>
...
SpaMTP has implemented a similar method to the metaspace.py python package, where users can directly load their chosen METASPACE dataset by simply providing the dataset id, fdr threshold and annotation database (the user may also provide their API key to access private datasets).


Below we will demonstrate how to load the public [moues liver with spotted chemical standards dataset](https://metaspace2020.org/annotations?db_id=38&ds=2020-12-07_03h16m14s).
...
df <- metaspace@assays$Spatial@meta.data %>%
  arrange(desc(msm)) %>%
  slice_head(n = 4) %>% 
  select(-chemMod, -neutralLoss)
...
Based on the fdr limit of 5% we can import all relevant peaks. Lets visualise `Glutathione`: 
...
<iframe src="https://metaspace2020.org/annotations?db_id=38&ds=2020-12-07_03h16m14s" width="100%" height="600px"></iframe>

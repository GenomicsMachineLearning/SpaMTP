...
This function is designed for plotting 2 features simultaneously. If we only want to visualise one of the features we can use a work around to only show a single trace, where trace 1 = feature1, 2 = feature2 and 3 = image. Here we just provide one feature to plot. To stop this one feature being replicated we can add an additional line of code shown below:

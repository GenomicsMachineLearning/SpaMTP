...
This vignette will use various datasets, mainly focusing on the public mouse bladder SM data first published by [Römpp et al.(2010)](https://doi.org/10.1002/anie.200905559), and used in the [*Cardinal V3* article](https://doi.org/10.1038/s41592-023-02070-z). 
...
There are three main ways to load data with *SpaMTP*. To demonstrate each we will use three different public datasets. 


### 1. Converting From Cardinal Object

The first method is to convert a already loaded *Cardinal* object directly to a *SpaMTP* object. This will allow users to analyse any pre-existing cardinal dataset using SpaMTP, and also use pre-processing tools implmeneted by both packages. To demonstrate this we will load, process and convert the 'PIGII_206' [pig fetus dataset](https://bioconductor.org/packages/release/data/experiment/vignettes/CardinalWorkflows/inst/doc/MSI-segmentation.html).


```{r warning=FALSE, message=FALSE}
pig206 <- readRDS("~/Downloads/pig206.RDS")
```
...
From this we can see areas such as the liver, heart, and brain distinguished as segments 4, 1, and 5 as presented in the original *Cardinal* vignette. 
...
1. pig load cardinal, processes, covert
2. rcc load already processed imzML file
3. bladder mtx

```{r}
msi <- ReadSM_mtx("~/Downloads/bladder_csv.csv", mz.prefix = "mz-", feature.start.column = 2)
...
Pre-processing is an import step in any pipeline and should be performed prior to any downstream analysis. For spatial metabolic data, there are various packages which have robust functions and applications for removing technical noise from a SM dataset, these include *Cardinal* and *SCILS-LAB*. *SpaMTP* also provides general pre-processing functions such as data normalisation, scaling and filtering. 

For the following tutorial we will use processed data from [*Cardinal's* V3 Vignette (2023)](https://github.com/Vitek-Lab/Cardinal3-vignettes). The raw data is from, "[Histology by Mass Spectrometry: Label-Free Tissue Characterization Obtained from High-Accuracy Bioanalytical Imaging](https://doi.org/10.1002/anie.200905559)",
...
The first step is loading the data. In this case we are using already pre-processed data generated from [*Cardinal's* V3 Vignette (2023)](https://github.com/Vitek-Lab/Cardinal3-vignettes). The data was saved in the format of .ibd and .imzML files already binned to a resolution of 10ppm. Alternative formats that data can be loaded into SpaMTP are through an intensity matrix or a Cardinal Object. These are displayed in different vignettes.



Here, we have specified the file path were the .imzML and .imb files are stored, along with the mass range of m/z values to include.


```{r warning=FALSE, message=FALSE}
bladder <- LoadSM(file = "vignette_data_files/Mouse_Urinary_Bladder/mouse-bladder-peaks",mass.range=c(400,1000))
```
...
Based on the image above, there are a few clusters that are associated with regions outside the tissue. Because we are not interested in these (technical noise), we will subset our SpaMTP object to only include the clusters of interest (cluster 1, 5 and 7).


```{r, fig.width = 20, fig.height = 8, dev.args = list(bg = "#ffffff")}
# subset the pixels to only include our ssc clusters of interest
tissue_clusters = c(1,5,7)
...
```{r, warning=FALSE, message=FALSE}
ROI <- subset(bladder_annotated, subset = ssc %in% c("1", "5", "7")) # subset our dataset to only include our 3 clusters (Region of Interest - ROI)
```
...
DEMsHeatmap(cluster_DEMs, only.pos = FALSE, order.by = "logFC",
            ...
            ,plot.save.height = 10, plot.save.width = 15, annotation_colors = col_palette[c("1","5","7")])
```
...
ROI_x <- subset(bladder_annotated, subset = ssc %in% c("5", "7"))

### Performs differential expression analysis between the two groups
cluster_DEMs_x <- FindAllDEMs(data = ROI_x, ident = "ssc", n = 10, logFC_threshold = 1.2, 
                            DE_output_dir =NULL, return.individual = FALSE, 
...
urothelium <- cluster_DEMs_x$DEMs[cluster_DEMs_x$DEMs$cluster == "7",]

### Sets up colouring for significant spots in volcano plot
keyvals <- ifelse(
    urothelium$logFC < -2  & urothelium$P.Value < 10e-4, 'royalblue',
      ifelse(urothelium$logFC > 1 & urothelium$P.Value < 10e-4, 'red',
        'black'))
  keyvals[is.na(keyvals)] <- 'black'
  names(keyvals)[keyvals == 'red'] <- 'Cluster 7'

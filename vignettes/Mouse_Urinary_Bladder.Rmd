...
* db: Database to query. The default `NULL` loads the bundled RaMP 3.0.7 chemical-property table and preserves current `RAMP_C_*` identifiers for pathway analysis. A custom database can still be supplied.
...
The indexed annotation uses the bundled RaMP chemical properties. All ppm-valid candidates are stored (`min_score = 0`), so downstream pathway analysis and the interactive network viewer can apply a user-defined score threshold without re-running annotation.

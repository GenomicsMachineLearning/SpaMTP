---
title: "SpaMTP: Mouse Urinary Bladder Vignette"
...
In addition to storing the m/z annotations in the feature meta.data slot, if save.intermediates = `TRUE` is set within the *AnnotateSM()* function then an intermediate data.frame will be stored in the `@tools` slot of the SpaMTP object. This data.frame is a less compressed version of the annotation data.frame stored in the feature meta.data slot which is used later for pathway analysis. For more infomation visit the hidden window below:
...
```{r, fig.width=10, fig.height=30, warning=FALSE, message=FALSE}
...
SpaMTP also includes a useful density plot function that generates a [HTML](https://github.com/GenomicsMachineLearning/SpaMTP/blob/main/vignettes/vignette_data_files/Mouse_Urinary_Bladder/mzs_density_map.html) which allows the user to simultaneously spatially visualise any m/z peak and it's relative metabolite annotation. Users have access to a mass intensity plot where they can select any m/z peak, which will then be plotted and all possible metabolite annotations will be listed below. Lets demonstrate:
...
Lets now visualise these results. The first results we will plot are the ones generated based on the provided metabolite ID's.
...
```{r echo=FALSE, results='hide',warning=FALSE, message=FALSE, fig.width=10, fig.height=30}
...
```{r echo=FALSE, results='asis',warning=FALSE, message=FALSE, fig.width=30, fig.height=10}
p
```
...
Based on this we can see that there is only one significantly differentially expressed pathway. Before we annotated our sample based on only `M+K` analytes, and using the LIPIDMAPS database. To increase the number of possible pathways detected we can broaden our annotation range to include "M+H" and "M+K" analytes from the HMDB, LIPIDMAPS and ChEBI databases and re-run the analysis. This time we will use *Seurat's* FindAllMarker() function to calculating DE metabolites, highlighting ***SpaMTP's*** flexibility.
...
## Re-run DE analysis
DE <- FindAllDEPs(data = ROI_pathays, ident = "ssc", n = 3, logFC_threshold = 1.2, 
                            DE_output_dir =NULL, return.individual = FALSE, 
                            run_name = "FindAllDEPs", annotation.column = "all_IsomerNames")

DE_pathways <- FindRegionalPathways(SpaMTP = ROI_pathays,
                                ident = "ssc",
                                DE.list = list(DE$DEPs),
                                analyte_types = c("metabolites"),
                                SM_assay = "Spatial",
                                ST_assay = NULL)
```
...
This plot highlights that cluster 5 (bladder muscle) is enriched for `Biochemical pathways`. Alternatively, the Urothelium (cluster 6) is shown to down regulate numerous pathways including `Sphingolipid metabolism`. This matches with previous research suggesting that the metabolism of sphingolipids is maily associated with muscle cells of the badder. 

These results may not tell the entire story however, as the 'ssc' clustering results do not capture every region of the tissue according to the original publication. There is a specific area between the bladder and urothelium called the 'lamina propria' which is currently unidentifiable based on the 'ssc' clustering result (this was also mentioned in the [original paper](https://doi.org/10.1038/s41592-023-02070-z)). 
...
We can also run pathway analysis on the new clustering results. Lets do that now, this time we will use *Seurat's* FindAllMarker() function to show ***SpaMTP's*** flexability.

```{r}

ROIx <- AnnotateSM(ROI, db = rbind(Chebi_db,HMDB_db), polarity = "positive")

## Re-run DE analysis
DE <- FindAllDEPs(data = ROIx, ident = "clusters", n = 3, logFC_threshold = 1.2, 
                            DE_output_dir =NULL, return.individual = FALSE, 
                            run_name = "FindAllDEPs", annotation.column = "all_IsomerNames")

DE_pathways <- FindRegionalPathways(SpaMTP = ROIx,
                                ident = "clusters",
                                DE.list = list(DE$DEPs),
                                analyte_types = c("metabolites"),
                                SM_assay = "Spatial",
                                ST_assay = NULL,pval_cutoff_pathway = 0.01)

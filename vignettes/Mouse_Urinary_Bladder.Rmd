...
pig206 <- readRDS("~/Documents/SpaMTP_Resubmission/DataObjects/pig206.RDS")
```
...
::::: columns
::: column
```{r, fig.width=10, fig.height=10, dev.args=list(bg="#ffffff"), warning=FALSE, message=FALSE}
image(pig206_peaks, mz=537.08)
...
::: column
```{r, fig.width=8, fig.height=5,, dev.args=list(bg="#ffffff"), warning=FALSE, message=FALSE}
...
```{r, warning=FALSE, message=FALSE}
rcc_spamtp <- LoadSM(file = "~/Documents/SpaMTP_Resubmission/DataObjects/rcc/rcc", mass.range = c(150, 1000), multi.run = T)
```
...
```{r warning=FALSE, message=FALSE}
bladder <- ReadSM_mtx("~/Documents/SpaMTP_Resubmission/DataObjects/bladder_csv.csv", mz.prefix = "mz-", feature.start.column = 2)
```
...
```{r warning=FALSE, message=FALSE}
bladder_metadata <- read.csv("~/Documents/SpaMTP_Resubmission/DataObjects/bladder_metadata.csv", row.names = 1)
```
...
```{r, fig.width=20, fig.height=5, warning=FALSE, message=FALSE}
raw_bladder <- readRDS("~/Documents/SpaMTP_Resubmission/DataObjects/raw_bladder.RDS") 
...
As an example, now that we have demonstrated our data is of good quality, we can convert our *SpaMTP* object to *Cardinal* to perform non-negative matrix factorization (NMF). This is method is similar to Principal component analysis (PCA) but produces non-negative loadings. Lets first convert our data:
...
We can see that the Object layout is correct and we have 305 m/z values and our seurat metadata has also been stored in the `pixelData` slot. Next we can run NMF and plot the results. 

```{r, warning=FALSE, message=FALSE, fig.width=20, fig.height=5 , dev.args = list(bg = "#ffffff")}
bladder_NMF <- NMF(bladder_cardinal, ncomp=3, niter=30)

image(bladder_NMF, smooth="adaptive", enhance="histogram",superpose = F,layout=c(1,3))
```

We can see these components follow a similar patter to our segments. Next we can generate a plot using Cardinal:

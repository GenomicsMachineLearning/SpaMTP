---
title: "Mouse_Urinary_Bladder"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Mouse_Urinary_Bladder}
...
bladder <- loadSM(name = "mouse-bladder-peaks", folder = "vignette_data_files/Mouse_Urinary_Bladder/", mass.range=c(400,1000), resolution=10, units="ppm")
```
...
```{r, warning=FALSE, message=FALSE, results='hide'}
PCA_results <- principal_component_pathway_analysis(ROI,
                                                path = "~/Downloads/",
                                                ppm_error = NULL,
                                                ion_mode = "positive",
...
This function returns a SpaMTP object with the relative PCA results, along with a list containing pathway analysis results. For ease of viewing this function also outputs an interactive HTML file containing all this information. Lets visualise this file below:

```{r, include=FALSE}
### TODO: get Ty to fix HTML code


### For displaying pathway analysis results
# Path to the HTML file
#file_path <- "/scratch/user/uqacause/files/project_data_objects/SpaMTP/pca_analysis.html"

# Read the HTML file
#html_content <- readLines(file_path, warn = FALSE)
#styled_html_content <- paste(
...
#display_html(styled_html_content)
```
...
```{r, warning=FALSE, message=FALSE, results='hide'}
cluster_6_pathways <- FisherexactTest(list(mz = cluster_6$gene),
                            analyte_type = c("mz"),
                            polarity = "positive",
                            ppm_error = 10,
                            max_path_size = 500,
...
```{r, warning=FALSE, message=FALSE}
ROI <- FindNeighbors(PCA_results$object, dims = 1:30, verbose = FALSE) # use the SpaMTP object generated from PCA Pathway Analysis
...
```{r, warning=FALSE, message=FALSE}
correlated_features <- FindCorrelatedFeatures(ROI, ident = "clusters", SM.assay = "Spatial", nfeatures = 10) #find the spatial correlation between each cluster and their relative metabolites
```

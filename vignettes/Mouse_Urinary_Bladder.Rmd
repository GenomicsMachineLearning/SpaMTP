...
```{r, include=FALSE}
rcc_file <- resolve_spamtp_vignette_file(
  article = "Mouse_Urinary_Bladder",
  filename = "rcc.RDS",
  url = "https://zenodo.org/api/records/17246588/files/rcc.RDS/content",
  expected_md5 = "0a7bb6ccc043aab4078f1c9f3fcb3570"
)
rcc_spamtp <- readRDS(rcc_file)
```
...
* db: Database to query. The default `NULL` resolves the versioned RaMP 3.0 chemical-property table through `SpaMTPdb` and preserves current `RAMP_C_*` identifiers for pathway analysis. A custom database can still be supplied.
...
The indexed annotation uses the current versioned RaMP chemical properties from `SpaMTPdb`. All ppm-valid candidates are stored (`min_score = 0`), so downstream pathway analysis and the interactive network viewer can apply a user-defined score threshold without re-running annotation.
...
In addition to storing the m/z annotations in the feature meta.data slot, if `save.intermediate = TRUE` is set within *AnnotateSM()*, the scored candidates and their RaMP provenance are stored in `@tools$mz_annotation`. A compatibility copy remains in `@tools$db_3`. The scored result is used later for pathway analysis. For more information visit the hidden window below:
...
The example uses `pval_cutoff_mets = 1`, so annotated metabolites can contribute to pathway ranks regardless of DE significance. Set it back to `0.05` for the stricter DE-significant analysis. To increase annotation coverage further, we can broaden the adduct range from `M+K` to both `M+H` and `M+K` and re-run the analysis.

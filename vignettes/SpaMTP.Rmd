...
The companion `SpaMTPData` package provides named, cached access to the
experiment objects used in these vignettes through ExperimentHub-compatible
metadata. Until its Bioconductor resources are published, it resolves the same
immutable files from the [*SpaMTP* Zenodo community](https://zenodo.org/communities/spamtp/records).

``` r
SpaMTPData::SpaMTPDataResources()
striatum <- SpaMTPData::SpaMTPData("striatum_dhb")
```

The separate `SpaMTPdb` package contains versioned, pruned RaMP database
resources. SpaMTP loads it automatically when installed; use
`SpaMTPDatabaseInfo()` to inspect the active resource catalogue.

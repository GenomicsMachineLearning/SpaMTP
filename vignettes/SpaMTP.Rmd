...
Each tutorial provides direct download links for its experiment objects from
the [*SpaMTP* Zenodo community](https://zenodo.org/communities/spamtp/records),
with checksum-verified downloads where automated fetching is used. No
companion data package is required for this standalone release.

The pruned RaMP annotation databases and corrected pathway graphs are included
with SpaMTP. On the developmental branch, `SpaMTPDatabaseInfo()` lists the
bundled resources and `LoadSpaMTPDatabase()` reads them offline.

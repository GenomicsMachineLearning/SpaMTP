...
vignette: >
  %\VignetteIndexEntry{SpaMTP: Spatial Multi-Omics Analysis}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
spamtp_root <- if (file.exists("DESCRIPTION")) {
  "."
} else if (file.exists(file.path("..", "DESCRIPTION"))) {
  ".."
} else {
  stop("Cannot locate the SpaMTP package root for vignette caching.")
}
mouse_brain_data_dir <- file.path(
  normalizePath(spamtp_root), "vignettes", "vignette_data_files", "Mouse_Brain"
)
mouse_brain_download_cache <- file.path(
  normalizePath(spamtp_root), "vignettes", ".knitr-cache", "downloads"
)
resolve_mouse_brain_demo_file <- function(relative_path,
                                           url = NULL,
                                           expected_md5 = NULL) {
  is_valid <- function(path) {
    if (!file.exists(path) || file.info(path)$size <= 0) return(FALSE)
    is.null(expected_md5) || identical(
      unname(tools::md5sum(path)), expected_md5
    )
  }

  repository_file <- file.path(mouse_brain_data_dir, relative_path)
  if (is_valid(repository_file)) return(repository_file)

  cached_file <- file.path(mouse_brain_download_cache, basename(relative_path))
  if (is_valid(cached_file)) return(cached_file)
  if (is.null(url)) {
    stop("Missing Mouse Brain demo file: ", repository_file)
  }

  dir.create(mouse_brain_download_cache, recursive = TRUE, showWarnings = FALSE)
  ...
  for (attempt in seq_len(5L)) {
    ...
    if (is.null(last_error) && is_valid(partial_file)) {
      if (!file.rename(partial_file, cached_file)) {
        stop("Could not finalize downloaded demo file: ", cached_file)
      }
      return(cached_file)
    }
    if (attempt < 5L) Sys.sleep(15L * attempt)
  }

  stop(
    "Failed to download or validate ", basename(relative_path),
    " after 5 attempts",
    if (!is.null(last_error)) paste0(": ", conditionMessage(last_error)) else ""
  )
}
cache_inputs <- c(
  file.path(spamtp_root, "DESCRIPTION"),
  file.path(spamtp_root, "NAMESPACE"),
  list.files(file.path(spamtp_root, "R"), pattern = "\\.R$", full.names = TRUE)
)
# knitr already includes each chunk's code in its individual cache key. A
# stable schema token prevents a prose or display-only vignette edit from
# invalidating every expensive analysis chunk. Bump this token only when the
# cache structure itself changes. Its initial value preserves the cache key
# used before this policy was introduced.
spamtp_cache_schema <- "52d41ddee15de27d449bc553ed7e748b"
spamtp_cache_key <- paste(
  R.version$major,
  R.version$minor,
  paste(
    c(unname(tools::md5sum(cache_inputs)), spamtp_cache_schema),
    collapse = "-"
  ),
  sep = "-"
)
knitr::opts_chunk$set(
  ...
  cache.path = file.path(
    normalizePath(spamtp_root), "vignettes", ".knitr-cache",
    "Multi-Omic_Mouse_Brain", ""
  ),
  cache.extra = spamtp_cache_key,
  cache.lazy = FALSE,
  cache.rebuild = identical(
    tolower(Sys.getenv("SPAMTP_REBUILD_VIGNETTES")), "true"
  )
)
```
...
```{r fig.width=7,dpi=72, fig.height=5}
...
```{r, warning=FALSE, message=FALSE, dpi=72,fig.width=10, fig.height=6}
...
```{r, include=FALSE}
visium_file <- resolve_mouse_brain_demo_file(
  file.path("ST_data", "visium.RDS"),
  url = "https://zenodo.org/api/records/17246900/files/visium.RDS/content",
  expected_md5 = "fe1beef8631cf16d320334ab3694f58f"
)
vis <- readRDS(visium_file)
vis <- UpdateSeuratObject(vis)
```
...
```{r fig.width=10, dpi=72,fig.height=6}
...
```{r,dpi=72, fig.width=20, fig.height=6}
...
```{r, fig.width=7,dpi=72, fig.height=4}
...
```{r, fig.width=15, fig.height=8 ,dpi=72}
...
```{r, include=FALSE}
gc()
msi_transformed_file <- resolve_mouse_brain_demo_file(
  file.path("SM_data", "SpaMTP_msi_transformed.RDS"),
  url = paste0(
    "https://zenodo.org/api/records/17246900/files/",
    "SpaMTP_msi_transformed.RDS/content"
  ),
  expected_md5 = "4c9e3db3412df2a255a58fc6939e37de"
)
msi_transformed <- readRDS(msi_transformed_file)
# The serialized demo predates the scored RaMP annotation store. Re-annotation
# replaces its legacy @tools$db_3-only result while preserving every feature.
msi_transformed <- AddFMP10Annotations(
  msi_transformed,
  add.custom.annotation = fmp10_metabolites,
  assay = "Spatial",
...
```{r, fig.width=10, fig.height=8, dev.args = list(bg = "#ffffff"),dpi=72}
...
```{r, fig.width=15, fig.height=7,dpi=72, warning=FALSE, message=FALSE}
...
```{r, fig.width=20, fig.height=6, ,dpi=72}
...
```{r, fig.width=20, fig.height=8,dpi=72 }
...
```{r, warning=FALSE, message=FALSE, fig.width=15, fig.height=5,dpi=72}
...
```{r st_marker_de, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r, fig.width=15, fig.height=5,dpi=72}
...
```{r sm_clustering, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r, fig.width=25, fig.height=8,dpi=72}
...
```{r, fig.width=7, fig.height=4,dpi=72}
...
```{r spatial_graph_pca, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r fig.width=10, fig.height=8,dpi=72}
# Non-Spatial Plots
...
```{r multiomic_neighbors, cache=TRUE}
...
```{r multiomic_umap_clusters, cache=TRUE}
...
```{r cluster_ari, warning=FALSE, message=FALSE, results='hide', cache=TRUE}
...
```{r integrated_dem, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r, fig.width=7, fig.height=7, message=FALSE, warning=FALSE, ,dpi=72}
...
df <- head(striatum_DEGs[striatum_DEGs$cluster == "2",], n = 4)
if (nrow(df) == 0L) {
  cat("<p><em>No cluster 2 markers passed the current differential-expression thresholds.</em></p>")
} else {
  df %>%
      knitr::kable(format = "html", table.attr = "class='dataframe'", align = "c") %>%
      kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
      ...
      kableExtra::row_spec(0, bold = TRUE) %>%
      kableExtra::row_spec(seq_len(nrow(df)), extra_css = "border-bottom: 1px solid #ddd; border-top: 1px solid #ddd;")
}
cat('</div>')
```


When markers pass the current differential-expression thresholds, the demo
typically recovers "Pcp4" among the genes up-regulated in cluster 2 (intact
striatum cells), consistent with the original publication.


```{r, fig.width=7, fig.height=4, message=FALSE, warning=FALSE,dpi=72}
...
```{r fmp_pathway_analysis, results='hide', warning=FALSE, message=FALSE, cache=TRUE}
...
```{r, fig.width=15, fig.height=10, warning=FALSE, message=FALSE,dpi=72}
...
```{r dhb_load_annotation, warning=FALSE, message=FALSE, cache=TRUE}
...
<!-- Plotting stays outside the cached chunk above. knitr caches R objects but not
the figure files a chunk writes, so a cached plotting chunk replays its <img> tag
without rewriting the PNG and pkgdown then aborts on the dangling reference. -->
```{r dhb_striatum_plot, warning=FALSE, message=FALSE, fig.width=7, fig.height=4, dpi=72}
SpatialDimPlot(
  striatum.dhb.data,
  group.by = "striatum",
  pt.size.factor = 2
...
```{r dhb_pathway_analysis, results='hide', warning=FALSE, message=FALSE, cache=TRUE}
...
One of the significant pathways associated with the lesioned region was 'Dopamine beta-hydroxylase deficiency'. We can use ***SpaMTP's*** network plotting function to visualise the network of this pathway, along with the relative differentially expressed elements that make up the activity of this pathway. The embedded viewer below is generated from the current RaMP annotations during the vignette build.

```{r dhb_pathway_network, results='asis', warning=FALSE, message=FALSE}
...
```{r, fig.width=7, fig.height=4,warning=FALSE, message=FALSE,dpi=72}
...
```{r st_geseca, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r, fig.width=10, fig.height=5,warning=FALSE, message=FALSE,dpi=72}
## Get top 6 pathways
...
```{r sm_geseca, warning=FALSE, message=FALSE, cache=TRUE}
...
Pathway coverage can differ between modalities. If a selected pathway has no
matching features in an assay, its panel states this explicitly. Such a panel
indicates unavailable evidence, not zero pathway activity.

```{r, fig.width=10, fig.height=4,dpi=72,warning=FALSE, message=FALSE}
...
```{r multiomic_geseca, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r, fig.width=10, fig.height=8,dpi=72,warning=FALSE, message=FALSE}
#key_pathways <- c("Parkinson disease", "Dopamine beta-hydroxylase deficiency", "Dopamine metabolism")

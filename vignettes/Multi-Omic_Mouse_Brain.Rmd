...
```{r, echo=TRUE, results='hide'}
msi_custom_annotated[["Spatial"]]@meta.data[410:415,]
```

```{r, echo=FALSE}
df <- msi_custom_annotated[["Spatial"]]@meta.data[410:415,]
df %>%
    knitr::kable(format = "html", table.attr = "class='dataframe'", align = "c") %>%
    kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
                  full_width = F, position = "center") %>%
...
integrated.data <- FindClusters(integrated.data, graph.name = "wsnn", algorithm = 3, resolution = 1.3,  cluster.name = "integrated_clusters", verbose = FALSE)
```
...
results <- cluster_DEMs$DEMs[cluster_DEMs$DEMs$cluster == "5",]
...
keyvals <- ifelse(
  ...
  names(keyvals)[keyvals == 'red'] <- 'Cluster 5 - UP'
  names(keyvals)[keyvals == 'black'] <- 'Non-Sig'
  names(keyvals)[keyvals == 'orange'] <- 'Dopamine'
  names(keyvals)[keyvals == "royalblue"] <- "Cluster 5 - Down"
...
```{r, echo=TRUE, results='hide'}
head(striatum_DEGs[striatum_DEGs$cluster == "5",], n = 4)
```

```{r, echo=FALSE}
df <- head(striatum_DEGs[striatum_DEGs$cluster == "5",], n = 4)
df %>%
    knitr::kable(format = "html", table.attr = "class='dataframe'", align = "c") %>%
    kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
                  full_width = F, position = "center") %>%
...
# Generate differential abundance and expression results for integrated clustering
Idents(striatum) <- "lesion"
deg =  FindAllMarkers(striatum,  assay = "SPT",only.pos =F)
dem =  FindAllMarkers(striatum , assay = "SPM", only.pos =F)

ids <- striatum@assays$SPM@meta.data[c("mz_names", "all_Isomers_IDs")]
dem$mz_names <- dem$gene
dem <- merge(dem, ids, by = "mz_names")

## Run DE analysis for DHB data
Idents(striatum.dhb.data) <- "lesion"
deg_dhb =  FindAllMarkers(striatum.dhb.data ,  assay = "SPT",only.pos =F)
dem_dhb =  FindAllMarkers(striatum.dhb.data , assay = "SPM", only.pos =F)

ids <- striatum.dhb.data@assays$SPM@meta.data[c("mz_names", "all_Isomers_IDs")]
dem_dhb$mz_names <- dem_dhb$gene
dem_dhb <- merge(dem_dhb, ids, by = "mz_names")

degs <- do.call(rbind, list(deg, deg_dhb))
dems <- do.call(rbind, list(dem, dem_dhb))

DE.up = list("genes" = degs[degs$cluster == "intact" & degs$avg_log2FC > 0 & degs$p_val_adj < 0.05, ],"metabolites" = dems[dems$cluster == "intact"& dems$avg_log2FC > 0 & dems$p_val_adj < 0.05, ])
DE.down = list("genes" = degs[degs$cluster == "intact" & degs$avg_log2FC < 0 & degs$p_val_adj < 0.05, ],"metabolites" = dems[dems$cluster == "intact"& dems$avg_log2FC < 0 & dems$p_val_adj < 0.05, ])

filtered_metabolites <- unique(unlist(strsplit(DE.up$metabolites$all_Isomers_IDs, "; ")))
filtered_metabolites <- filtered_metabolites[!filtered_metabolites %in% c("", "No Annotation")]

FishersPathwayAnalysis(list("genes" = DE.up$genes$gene, "metabolites" = filtered_metabolites))

...
In this case, the matrix run was an FMP10 matrix, meaning the masses of our metabolites have been previously annotated. We will load this file in and add the results to our SpaMTP SM object.
...
The function above adds these annotations into the SpaMTP object's feature meta.data slot. We can see this below:
...
combined.data <- MapSpatialOmics(SM.data = msi_transformed, ST.data = vis, overlap.threshold = 0.1, annotations = TRUE, ST.scale.factor = NULL)
```
...
## Run SM clustering 
combined.data <- NormalizeSMData(combined.data, assay = "SPM")
combined.data <- FindVariableFeatures(combined.data, verbose = FALSE)
combined.data <- ScaleData(combined.data, verbose = FALSE)
...
combined.data <- FindClusters(combined.data, resolution = 0.3,  cluster.name = "SM_clusters", verbose = FALSE)
```
...
integrated.data <- MultiOmicIntegration(combined.data,return.intermediate = T, weight.list = list(0.4,0.6),
                                        reduction.list =  list("spt.pca", "spm.pca"), dims.list = list(1:30, 1:30))
```


```{r}
integrated.data <- RunUMAP(integrated.data, nn.name = "weighted.nn", reduction.name = "wnn.umap", reduction.key = "wnnUMAP_",verbose = FALSE)
integrated.data <- FindClusters(integrated.data, graph.name = "wsnn", algorithm = 1, resolution = 1,  cluster.name = "integrated_clusters", verbose = FALSE)
```
...
striatum <- subset(integrated.data, subset = integrated_clusters %in% c("2", "3"))

striatum_DEGs <- FindAllMarkers(striatum, only.pos = T)
```

```{r, echo=TRUE, results='hide'}
head(striatum_DEGs[striatum_DEGs$cluster == "2",], n = 5)
```

```{r, echo=FALSE}
df <- head(striatum_DEGs[striatum_DEGs$cluster == "2",], n = 5)
df %>%
    knitr::kable(format = "html", table.attr = "class='dataframe'", align = "c") %>%
    kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
                  full_width = F, position = "center") %>%
...
Now that we have identified there are metabolic and transcriptional differences between the lesioned and intact striatum regions, we can now further assess the activity of each region by performing multi-modal pathway analysis using both genes and metabolites. Due to the nature of the FMP10 matrix, there are only a handful of metabolites annotated and all additional m/z values will **currently** not match the mass values found in any of the ***SpaMTP*** provided databases (due to the additional weight of the FMP10 adduct). Instead, we will use paired Visium and MALDI DHB-matrix data. This data has already been processed with the code hidden below:
